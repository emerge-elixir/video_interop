...
```sh
mix hex.build --unpack --output /tmp/video_interop-0.1.2
...
```sh
git tag -a v0.1.2 -m "Release VideoInterop 0.1.2"
git push origin main
git push origin v0.1.2
```

Wait for every CI job on `v0.1.2` to pass. Do not publish from a different
checkout or commit.

## Publish the Rust crate through CI

Pushing `v0.1.2` starts the normal CI matrix. The `publish-crate` job depends on
...
After crates.io accepts 0.1.2, create a temporary crate that depends on
`video-interop = "=0.1.2"`. Fetch it from crates.io and compile the default,
...
Create the GitHub release from `v0.1.2`. Include the supported Vulkan scope and
